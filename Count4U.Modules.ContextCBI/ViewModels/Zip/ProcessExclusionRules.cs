...
namespace Count4U.Modules.ContextCBI.ViewModels
{
    ...
    public class ProcessExclusionRules
    {
        private const string MainDbSdf = "MainDB.sdf";

        private readonly IDBSettings _dbSettings;

        private readonly List<Func<ProcessCondition, bool>> _rulesPack;
       // private readonly List<Func<ZipUnpackCondition, bool>> _rulesUnpack;

		public ProcessExclusionRules(IDBSettings dbSettings)
        {
            _dbSettings = dbSettings;
            _rulesPack = new List<Func<ProcessCondition, bool>>();
        ...
        public bool IsAcceptableForPack(ZipRootFolder zipRootFolder, string path, int level)
        {
            ...
            if (this._rulesPack.Any(r =>
                                    {
                                        var condition = new ProcessCondition() { RootFolder = zipRootFolder, Path = path, Level = level};
