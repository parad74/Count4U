...
{
    public class InventorEditOptionsViewModel : CBIContextBaseViewModel
    {
		...
		public InventorEditOptionsViewModel(IEventAggregator eventAggregator,
        ...
        public override void OnNavigatedTo(NavigationContext navigationContext)
        {
            ...
            if (this._dynamicColumnsViewModel != null)
            {
				...
				this._dynamicColumnsViewModel.SetInventor(_inventor, enInventorAdapterInherit.InheritFromCustomer);
            }

			//ExportPdaSettingsInventorEdit
			//private ExportPdaSettingsViewModel _exportPdaSettingsViewModel;

			this._exportPdaSettingsViewModel = Utils.GetViewModelFromRegion<ExportPdaSettingsViewModel>(Common.RegionNames.ExportPdaSettingsInventorEdit, this._regionManager);
			if (this._exportPdaSettingsViewModel != null)
			{
				_exportPdaSettingsViewModel.IsEditable = !_isReadOnly;
				_exportPdaSettingsViewModel.SetIsShowConfig(false);
				_exportPdaSettingsViewModel.SetInventor(_inventor, enInventorAdapterInherit.InheritFromCustomer);
			}

			//ExportPdaSettingsInventorEdit
			//private ExportPdaSettingsViewModel _exportPdaSettingsViewModel;

			//AdditionalSettingsInventorEdit
			//private AdditionalSettingsSettingsViewModel _additionalSettingsSettingsViewModel;
			this._additionalSettingsSettingsViewModel = Utils.GetViewModelFromRegion<AdditionalSettingsSettingsViewModel>(Common.RegionNames.AdditionalSettingsInventorEdit, this._regionManager);
			if (this._additionalSettingsSettingsViewModel != null)
			{
				_additionalSettingsSettingsViewModel.SetIsEditable(true);
				//this._additionalSettingsSettingsViewModel.SetIsShowConfig(false);
				_additionalSettingsSettingsViewModel.SetInventor(_inventor, enInventorAdapterInherit.InheritNothing);
			}
        ...
        private void SaveInventor()
        {
			...
			if (this._exportPdaSettingsViewModel != null)
			{
				_exportPdaSettingsViewModel.ApplyCustomerChangesFromInventor();
			...
			if (this._additionalSettingsSettingsViewModel != null)
			{
				_additionalSettingsSettingsViewModel.ApplyChanges();
			}
