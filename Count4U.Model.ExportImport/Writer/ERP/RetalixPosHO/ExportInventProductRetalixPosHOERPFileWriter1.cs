...
{
	public class ExportInventProductRetalixPosHOERPFileWriter1 : IExportInventProductFileWriter
	{
...
//Makat (col 11-24)
//Qty (col 26-33)  //6.2 without the dot
//Sequantial Number in Itur (col 35-36)
			...
			//if (iturAnalyzes.Makat == "0000000000154")
			//{
			//	double quantityEditDouble1 = Convert.ToDouble(iturAnalyzes.QuantityEdit);
			//}

			double quantityEditDouble = Convert.ToDouble(iturAnalyzes.QuantityEdit);
			string quantityEdit = "00000000";

			if (iturAnalyzes.UnitTypeCode == "10")
			{
				int quantityEdit00 = (int)(quantityEditDouble * 100);

				if (quantityEdit00 != 0)
				{
					quantityEdit = quantityEdit00.ToString().LeadingZero8();
				}
			}

			if (iturAnalyzes.UnitTypeCode == "00")
			{
				int quantityEditInt = (int)(quantityEditDouble);
				double quantityEditDelta = quantityEditDouble - (int)(quantityEditDouble);

				int add = 0;
				if (quantityEditDelta >= 0.5)
				{
					add = 1;
				}

				quantityEditInt = quantityEditInt + add;

				if (quantityEditInt != 0)
				{
					quantityEdit = quantityEditInt.ToString().LeadingZero8();
				}
			}
			else
			{
