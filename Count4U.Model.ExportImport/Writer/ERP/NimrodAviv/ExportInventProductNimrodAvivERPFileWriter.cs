...
{
	public class ExportInventProductNimrodAvivERPFileWriter : IExportInventProductFileWriter
	{
		...
		public void AddRowSimple(StreamWriter sw, IturAnalyzesSimple iturAnalyzes, long countRow, string ERPNum = "", string INVDate = "", string separator = ",")
		{
			...
			string barcode = iturAnalyzes.Barcode.PadRight(15, ' ');//15 chars – Add spaces to the right
			...
			string color = "000";
			string size = "000";
			string old = "OLD  ";//Const "OLD" - 5 chars – Add spaces to the right
			string itemName = "".PadRight(32,' '); //32 Chars (Add spaces to the right)
			if (iturAnalyzes.IturCode.Length > 32)
			{
				itemName = iturAnalyzes.ProductName.Substring(0,32);
			}
			else
			{
				itemName = iturAnalyzes.ProductName.PadRight(32, ' ');
			}
			...
			//Size – 3 Chars (Col 13-15)
				...
				//Item Code (Makat)Barcode (original code From PDA)				//15 chars – Add spaces to the right
				...
				string[] newRows = new string[] { localBranchCode, iturCodeSuffix, barcode, quantityEdit, makatType, section, brand, color, size, old, itemName };
