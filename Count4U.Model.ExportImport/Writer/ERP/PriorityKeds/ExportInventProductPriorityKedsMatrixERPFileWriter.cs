...
{
	public class ExportInventProductPriorityKedsMatrixERPFileWriter : IExportInventProductFileWriter
	{
...
//First row:
//Type – Const1 (col 1)
//Makat-Part1 (col 3-8) -  Aligned to left (Cut first 6 chars from the Makat)
//Second row:
//Type – Const1 (col 2)
//Makat-Part2 (col 3-5) – Next 3 chars (Chars 7,8,9)
...
//2 208      3
			...
			string makat2 ="";
			if (makat.Length > 6) {
				makat1 = makat.Substring(0, 6);
				//int len = makat.Length - 6;
				makat2 = makat.Substring(6, 3); 
			}
...
//Code ERP (col 12-15) – attach to the right
			...
			string header = "0 " +
				...
				" " +
				String.Format("{0,4}", erpCode); 					 //Code ERP (col 12-15) – attach to the right
			sw.WriteLine(header);
