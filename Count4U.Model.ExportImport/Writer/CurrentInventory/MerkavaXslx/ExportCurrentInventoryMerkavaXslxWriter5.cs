...
{
	public class ExportCurrentInventoryMerkavaXslxWriter5 : IExportCurrentInventoryWriter
	{
		...
		{
			//if (currentInventory.UnitTypeCode == "SN")
			//{
				//if (string.IsNullOrWhiteSpace(currentInventory.PropertyStr10Code) == true
				//	&& string.IsNullOrWhiteSpace(currentInventory.PropertyStr11Code) == true)
				//{
					if (details != null)
					{
						if (details.Count > 0)
							...
							if (details.Count > 0)
							{
								foreach (var detail in details)
								{
									...
									if (pInventor != null)
									{
										...
										{
											isDoFixSerialNumberLocal = pInventor.SerialNumberLocal.Trim().ToLower() == currentInventory.SerialNumberLocal.Trim().ToLower();
										}

										bool isDoFixSerialNumberSupplier = false;
										if (string.IsNullOrWhiteSpace(pInventor.SerialNumberSupplier) == false)
										{
											isDoFixSerialNumberSupplier = pInventor.SerialNumberSupplier.Trim().ToLower() == currentInventory.SerialNumberSupplier.Trim().ToLower();
										}
