...
{
	public class BranchDefaultParser : IBranchParser
	{
		...
		public BranchDefaultParser(IServiceLocator serviceLocator,
			ILog log)
		{
			...
			Dictionary<ImportProviderParmEnum, object> parms = null)
		{
			...
			IBranchRepository branchRepository = _serviceLocator.GetInstance<IBranchRepository>();
				...
				countExcludeFirstString))
			{
				...
				string ret = BranchValidate.CodeValidate(branchCodeLocal);
				...
				if (record.Length > 1)
				{
					newBranch.BranchCodeERP = record[1];
					ret = BranchValidate.CodeErpValidate(newBranch.BranchCodeERP);
					if (ret != String.Empty)  //Error
					{
						this._errorBitList.Add(new BitAndRecord
						{
				...
				if (record.Length > 2)
				{
					string ret1 = BranchValidate.NameValidate(record[2]);
					if (ret1 == String.Empty)
					{
						newBranch.Name = record[2].ReverseDosHebrew(invertLetter, rt2lf);
					}
					else
					{
						this._errorBitList.Add(new BitAndRecord
				...
				{
					branchRepository.UpdateBranchName(newBranch);
