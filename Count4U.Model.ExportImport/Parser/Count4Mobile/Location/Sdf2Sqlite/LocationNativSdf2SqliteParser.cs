...
{
	public class LocationNativSdf2SqliteParser : ILocationSQLiteParser
	{
		...
		public LocationNativSdf2SqliteParser(IServiceLocator serviceLocator,
			ILog log)
		{
			...
			Dictionary<ImportProviderParmEnum, object> parms = null)
		{
			...
			ILocationRepository _locationRepository = this._serviceLocator.GetInstance<ILocationRepository>();
			Dictionary<string, Location> _locationCodeDictionary =  _locationRepository.GetLocationDictionary(fromDBPath, true);
			//Dictionary<string, InventProduct> GetIPQuntetyEditIturCode(SelectParams selectParams, string pathDB) //to do 
			// можно заменить    	double totalQuantity = inventProductRepository.GetSumQuantityEditByIturCode(itur.IturCode, fromPathFile);
...
//[Location].LevelNum= [Itur]. LevelNum
				...
				try
				{
					...
					if (itur.Description == "Added automatic")
					{
						if (_locationCodeDictionary.ContainsKey(itur.ERPIturCode) == true)
						{
							Location location = _locationCodeDictionary[itur.ERPIturCode];
							newLocation.Description = location.Description;
							newLocation.Level1Name = location.Name;
						}
					}
					...
					newLocation.Level4Code = itur.Level4;
					newLocation.Level4Name = itur.Name4;
