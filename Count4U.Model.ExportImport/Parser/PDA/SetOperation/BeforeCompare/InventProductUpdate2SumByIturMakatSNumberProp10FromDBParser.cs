...
{
	//Code  = Itur + Makat + SNumber  + PropertyStr10 //было Barcode =   Itur + Makat + SNumber
	public class InventProductUpdate2SumByIturMakatSNumberProp10FromDBParser : InventProductParserBase, IInventProductSimpleParser
	{
		...
		public InventProductUpdate2SumByIturMakatSNumberProp10FromDBParser(
		...
		{
			//получить из БД [Itur][Makat][SNumber] = Sum InventProduct by Code (Makat|SerialNumber)	 in Itur
			...
			// Записать   "Sum InventProduct by [Makat][SNumber]" заменяя DocumentHeaderCode
			...
			//key = Makat + "|" + SerialNumber + "|" + IturCode
			//ToDictionary(k => k.Makat + "|" + k.SerialNumber + "|" + k.IturCode);
			Dictionary<string, InventProduct> inventProductByMakatAndSNAndProp10IturCodeDictionary = this._inventProductRepository.GetIPQuntetyByMakatAndSNAndProp10IturCode(null, dbPath);
			...
			//TODO	 теперь хранится не в Barcode, а в Code	  = Makat + "|" + SerialNumber
			// key = Code + "|" + IturCode
			// key = Makat + "|" + SerialNumber + "|" + IturCode
			foreach (KeyValuePair<string, InventProduct> keyValuePair in inventProductByMakatAndSNAndProp10IturCodeDictionary)
			{
				...
				string propertyStr10 = newInventProduct.IPValueStr10;
				string code = makat + "|" + serialNumber + "|" + propertyStr10 ; //!!!	   Ключ

				//TODO	 теперь хранится не в Barcode, а в Code	  = Makat + "|" + SerialNumber	  + "|" + propertyStr10
				// key = Code + "|" + IturCode
				// key = Makat + "|" + SerialNumber + "|" + propertyStr10 + "|" + IturCode
				...
				newInventProduct.Code = code;		  //!!!		   Ключ с которым дальше работаем 	    Code	  = Makat + "|" + SerialNumber	 + "|" + propertyStr10
