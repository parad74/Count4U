...
{
	//Code  = Itur + Barcode + SNumber + PropertyStr10 было //Barcode =   Itur + Makat + SNumber
	public class InventProductUpdate2SumByIturBarcodeSNumberProp10FromDBParser : InventProductParserBase, IInventProductSimpleParser
	{
		...
		public InventProductUpdate2SumByIturBarcodeSNumberProp10FromDBParser(
		...
		Dictionary<ImportProviderParmEnum, object> parms = null)
		{
			...
			Dictionary<string, InventProduct> inventProductByBarcodeAndSNAndProp10IturCodeDictionary = this._inventProductRepository.GetIPQuntetyByBarcodeAndSNAndProp10IturCode(null, dbPath);
			...
			//TODO	 теперь хранится не в Barcode, а в Code	  = Barcode + "|" + SerialNumber   + "|" + propertyStr10
			// key = Barcode + "|" + SerialNumber + "|" + propertyStr10 + "|" + IturCode
			foreach (KeyValuePair<string, InventProduct> keyValuePair in inventProductByBarcodeAndSNAndProp10IturCodeDictionary)
			{
				...
				string code = barcode + "|" + serialNumber + "|" + propertyStr10; //!!!	   Ключ

				//TODO	 теперь хранится не в Barcode, а в Code	  = barcode + "|" + SerialNumber + "|" + propertyStr10
				// key = barcode + "|" + SerialNumber + "|" + IturCode	+  "|" + propertyStr10
				...
				newInventProduct.Code = code;		  //!!!		   Ключ с которым дальше работаем 	    Code	  = barcode + "|" + SerialNumber  +  "|" + propertyStr10
