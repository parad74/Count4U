...
{
	//Code  = Itur + Makat  было Barcode =   Itur + Makat
	public class InventProductUpdate2SumByIturMakatFromDBParser : InventProductParserBase, IInventProductSimpleParser
	{
		...
		public InventProductUpdate2SumByIturMakatFromDBParser(
		...
		{
			//получить из БД [Itur][Makat] = Sum InventProduct by Makat
			...
			// Записать   "Sum InventProduct by MakatAndItur" заменяя DocumentHeaderCode
			...
			//ToDictionary(k => k.Makat + "|" + k.IturCode);
			Dictionary<string, InventProduct>  inventProductByMakatsAndIturCodeDictionary = this._inventProductRepository.GetIPQuntetyByMakatsAndIturCode(null, dbPath);
		...
		//	base._documentHeaderRepository.Insert(documentHeaders, dbPath);
			...
			//БЫЛО key = Makat + "|" + IturCode 

			//TODO
			// key = Code + "|" + IturCode		// Code= Makat !!!
			// key = Makat + "|" + IturCode
			foreach (KeyValuePair<string, InventProduct> keyValuePair in inventProductByMakatsAndIturCodeDictionary)
			{
				InventProduct newInventProduct = keyValuePair.Value;
				string makat = newInventProduct.Makat;
				string code = newInventProduct.Makat;  //!!!
				...
				newInventProduct.Code = makat; 			 //!!!		   Ключ с которым дальше работаем 	   // Code= Makat !!!

				newInventProduct.SerialNumber = "";
