...
{
	//Code  = Itur + Makat + SNumber  было Barcode =   Itur + Makat + SNumber
	public class InventProductUpdate2SumByIturMakatSNumberFromDBParser : InventProductParserBase, IInventProductSimpleParser
	{
		...
		public InventProductUpdate2SumByIturMakatSNumberFromDBParser(
		...
		Dictionary<ImportProviderParmEnum, object> parms = null)
		{
			...
			Dictionary<string, InventProduct> inventProductByMakatAndSNAndIturCodeDictionary = this._inventProductRepository.GetIPQuntetyByMakatAndSNAndIturCode(null, dbPath);
			...
			// key = Code + "|" + IturCode
			// key = Makat + "|" + SerialNumber + "|" + IturCode
			foreach (KeyValuePair<string, InventProduct> keyValuePair in inventProductByMakatAndSNAndIturCodeDictionary)
			{
				...
				string code = makat + "|" + serialNumber; //!!!	   Ключ
				...
				newInventProduct.Code = code;		  //!!!		   Ключ с которым дальше работаем 	    Code	  = Makat + "|" + SerialNumber
