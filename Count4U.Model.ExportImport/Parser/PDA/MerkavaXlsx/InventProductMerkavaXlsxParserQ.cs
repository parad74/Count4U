...
{
	public class InventProductMerkavaXlsxParserQ : InventProductParserBase, IInventProductSimpleParser
	{
		...
		public InventProductMerkavaXlsxParserQ(
...
//4 SerialNumberLocal
				...
				string serialNumber = record[4].CutLength(49);
				string serialNumberSupplier = "" ;
				string makat = record[2].CutLength(299);
				string productName = record[3].CutLength(99);
				...
				string[] idsQ = new string[] { "", makat, erpIturCode, serialNumberSupplier };	// 4 - составной ключ для SN
				string IDQ = idsQ.JoinRecord("|");
...
//4 SerialNumberLocal
				...
				if (dictionaryInventProductCode.ContainsKey(IDQ) == true)
				{
					dictionaryInventProductCode[IDQ] = null; 
