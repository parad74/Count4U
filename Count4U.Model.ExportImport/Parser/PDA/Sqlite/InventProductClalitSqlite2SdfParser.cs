...
{
	public class InventProductClalitSqlite2SdfParser : InventProductParserBase, IInventProductSimpleParser
	{
		...
		public InventProductClalitSqlite2SdfParser(
			...
			Dictionary<ImportProviderParmEnum, object> parms = null)
		{
			...
			{
				// count4UDB
				dictionaryIturCodeERP = this._iturRepository.GetERPIturDictionary(dbPath);
			}
			catch { }
			...
			{
				// count4UDB
				//	dictionaryMakat_SerialNumber_IturCode = inventProductRepository.GetQuantitySerialInventProducts(dbPath);
				...
				countExcludeFirstString, tableName))
			{
				...
				else		   //row from table
				{
					...
					//KEY = Makat + SerialNumber + IturCodeERP
					//KEY = ItemCode + SerialNumberLocal + LocationCode
					//[0]	"Uid"	
					//[1]	     "SerialNumberLocal"	
					//[2]	"ItemCode"	
					//[3]	"SerialNumberSupplier"	
					...
					string[] ids = new string[] { serialNumber, makat, locationCodeFrom, propertyStrKey13 };	// 4 - составной ключ
					...
					string quantity = record[indexQuantity].CutLength(49);
					...
					newInventProduct.ERPIturCode = locationCodeFrom;
					...
					if (importType.Contains(ImportDomainEnum.ExistMakat) == true)
					{
						...
						{	// TODO: проверить
							newInventProduct.TypeMakat = TypeMakatEnum.W.ToString();		 //такого вообще не должно быть !!!
							newInventProduct.Makat = "111";		  //пока все один макат 111 
							newInventProduct.ProductName = "פריט לספירה"; 
							...
							newInventProduct.ImputTypeCodeFromPDA = SerialKey;
						}
					}		//ExistMakat
					else   //Not ExistMakat
					{
