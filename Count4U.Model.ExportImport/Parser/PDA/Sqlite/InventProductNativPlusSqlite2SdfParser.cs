...
{
	public class InventProductNativPlusSqlite2SdfParser : InventProductParserBase, IInventProductSimpleParser
	{
		...
		public InventProductNativPlusSqlite2SdfParser(
			...
			Dictionary<ImportProviderParmEnum, object> parms = null)
		{
				...
				countExcludeFirstString, tableName))
			{
				...
				else		   //row from table
				{
				   ...
				   //Nativ + 4 составной ключ 
					...
					//if (ID.Trim() == "0103073210|999999|5060-5081-11||")
					//{
					//	ID = ids.JoinRecord("|");
					//}

					//if (ID.Trim() == "SEHP211200343611|999999|5060-5081-2||")
					//{
					//	ID = ids.JoinRecord("|");
					//}
					...
					//string part1 = newInventProduct.GetPropertyValue("SupplierCode");
					//string part2 = newInventProduct.GetPropertyValue("IPValueStr8"); 
					...
					if (dictionaryInventProductCode.ContainsKey(ID) == true)
					{
						...
						if (oldInventProduct.ModifyDate < newInventProduct.ModifyDate)
						{
							...
							Log.Add(MessageTypeEnum.TraceParser, ID + "new DateModify: " + newInventProduct.ModifyDate
								+ "old DateModify: " + oldInventProduct.ModifyDate + "   " + filename);
							modifided++;
						}
						else
						{
		   ...
		   //===========================
			Log.Add(MessageTypeEnum.TraceParser, "Modifided: " + modifided + " from file  " + filename);
			...
			foreach (InventProduct inventProduct in inventProducts)
			{
				...
				//if (inventProduct.Barcode == @"HT122|PDA_HT682|003|1112276044|")
				//{
				//	;
				//}
