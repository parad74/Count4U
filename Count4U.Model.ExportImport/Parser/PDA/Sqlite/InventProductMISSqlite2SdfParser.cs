...
{
	public class InventProductMISSqlite2SdfParser : InventProductParserBase, IInventProductSimpleParser
	{
	//	protected Dictionary<string, string> _iturInFileDictionary; //key IturCode, DocumentCode - для файла
		protected Dictionary<string, DocumentHeader> _documentHeaderFromDBDictionary; //key IturCode, DocumentHeader 
		...
		public InventProductMISSqlite2SdfParser(
			...
			base( documentHeaderRepository, serviceLocator, log)
		{
			...
			this._documentHeaderFromDBDictionary = new Dictionary<string, DocumentHeader>();
			...
			Dictionary<ImportProviderParmEnum, object> parms = null)
		{
			...
			this._documentHeaderFromDBDictionary.Clear();
			...
			this._documentHeaderFromDBDictionary = documentHeaderRepository.GetDocumentHeaderDictionary(dbPath, true);
			...
			int indexQuantityInPackEdit = -1;
			//int indexPropertyStr1 = -1;//как в MIS	 Optional
				...
				countExcludeFirstString, tableName))
			{
				...
				if (rowCount == 1) // header of Table
				{
					...
					// 	dictionaryColumnNumbers - TODO получить индексы нужных полей, не найденых == -1
					{
						...
						indexQuantityInPackEdit = "QuantityInPackEdit".GetIndexColumnByName(dictionaryColumnNumbers, tableName, colCount, Log);
						//row[0]	"Uid"	
						...
						try
						{
							...
							//587f5dfe89b1f334^2018-337~b^2018_12_05_15_18_07^.db3
				...
				else		   //row from table
				{
					...
					if (newIturCode.IsNotNone() == true)    // не пустой			  
					{
						if (iturFromDBDictionary.IsDictionaryContainsKey(newIturCode) == false)          //IturCode нет таблице
						{
						...
						{
							//IturCode есть в таблице 	и не пустой			все хорошо
						}
					}
					else		// IturCode пустой   - работать с ERPIturCode // не должно, но допустим 
					{
						if (dictionaryIturCodeERP.ContainsKey(erpIturCode) == true) //и  Есть в текущей БД значит все ok 
						{
							...
							//this._iturDictionary[itur.IturCode] = itur;  // словарь для добавления в БД в репозитории после импорта всего файла
							iturFromDBDictionary[itur.IturCode] = itur;     // словарь для того чтобы не дублировать Itur.IturCode в БД
						}
						else
					...
					//	dbPath, newIturCode, newSessionCode, isNewItur);

					//словарь Iturs в текущем файле. Создаем для каждого Itur:File один DocumentHeader

					int currentIturInvStatus = 0;

					try
					{
						Itur currentItur = iturFromDBDictionary[newIturCode];
						currentIturInvStatus = currentItur.InvStatus;
					}
					catch (Exception exp)
					{
						Log.Add(MessageTypeEnum.Error, String.Format(ParserFileErrorMessage.IturCodeNotExistInDB, record.JoinRecord(separator)));
					}


					//словарь Iturs в текущем файле. Создаем для каждого Itur:File один DocumentHeader
					newDocumentCode = GetDocumentHeaderCodeByIturCode(ref IDDocumentHeader, dbPath, newDocumentCodePrefix, newIturCode, currentIturInvStatus, 
						newSessionCode, deviceName,	localUserName);
				...
				//	string[] ids = new string[] { serialNumber, makat, locationCodeFrom, serialNumberSupplier };	// 4 - составной ключ для SN
					...
					if (indexQuantityInPackEdit != -1)
					{
						newInventProductString.QuantityInPackEdit = record[indexQuantityInPackEdit];
					}
					...
					if (indexLocalUserName != -1)
					{
						newInventProduct.WorkerID = record[indexLocalUserName];
					}
		...
		private string GetDocumentHeaderCodeByIturCode(
			...
			string localUserName )
		{
			string retDocumentCode = newDocumentCodePrefix + "_" + newIturCode; 
			if (this._documentHeaderFromDBDictionary.ContainsKey(retDocumentCode) == true)
			{
				DocumentHeader document = this._documentHeaderFromDBDictionary[retDocumentCode];
				if (document != null) IDDocumentHeader = Convert.ToInt32(document.ID);
			...
			else // create new DocumentHeader
			{
				...
				else //	Error  retBitSession == 0 
				{
					...
					IDDocumentHeader = Convert.ToInt32(base._documentHeaderRepository.Insert(newDocumentHeader, dbPath));
					newDocumentHeader.ID = IDDocumentHeader;
					this._documentHeaderFromDBDictionary[retDocumentCode] = newDocumentHeader;
