...
namespace Count4U.Model.Count4U
{
	public class LocationParser : ILocationParser
	{
		...
		public LocationParser(IServiceLocator serviceLocator,
			ILog log)
		{
			...
			Dictionary<ImportProviderParmEnum, object> parms = null)
		{
				...
				countExcludeFirstString))
			{
				...
				if (objectType == "L")
				{
					...
					newLocationString.Code = recordEmpty[1];
					if (record.Length > 2)
					{
						newLocationString.Name = recordEmpty[2].ReverseDosHebrew(invertLetter, rt2lf);
					}
					if (record.Length > 3)
					{
						newLocationString.Description = recordEmpty[3];
					}
					if (record.Length > 4)
					{
						newLocationString.BackgroundColor = recordEmpty[4];
					}
