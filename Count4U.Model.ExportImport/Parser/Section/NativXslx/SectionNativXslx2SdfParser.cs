...
{
	public class SectionNativXslx2SdfParser : ISectionParser
	{
		...
		public SectionNativXslx2SdfParser(IServiceLocator serviceLocator,
			ILog log)
		{
			...
			Dictionary<ImportProviderParmEnum, object> parms = null)
		{
				...
				countExcludeFirstString))
			{
				...
				newSectionString.SectionCode = DomainUnknownCode.UnknownSection;
				if (string.IsNullOrWhiteSpace(record[5].Trim()) == false)
				{
					newSectionString.SectionCode = record[5].Trim(); //record[4];
				}

				newSectionString.Description = "";
				newSectionString.Name = DomainUnknownName.UnknownSection;
				if (record.Length > 6) //>5
				{
					if (string.IsNullOrWhiteSpace(record[6].Trim()) == false)
					{
