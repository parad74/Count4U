...
{	  //парсим ImportInfrastructure.Xslx и записываем Count4U.Itur 
	public class IturNativXslx2SdfParser : IIturParser
	{
		...
		public IturNativXslx2SdfParser(
			...
			Dictionary<ImportProviderParmEnum, object> parms = null)
		{
				...
				sheetNumberXlsx))
			{
				...
				{
					//Itur oldItur = dictionaryIturCodeERP[iturCodeERP];			//   23.02.2021
					//this._iturDictionary[oldItur.IturCode] = oldItur;					//	23.02.2021
				...
				{
						Log.Add(MessageTypeEnum.Error, "Can't insert in db Itur [" + iturCodeERP + "] Itur suffix too long");
						continue;
				}
				...
				//newIturString.Name = record[5].CutLength(49);		  //Level2Name
			...
			//foreach (KeyValuePair<string, Itur> keyValueItur in this._iturDictionary)	   // источник db3		  23.02.2021
			foreach (KeyValuePair<string, Itur> keyValueItur in iturFromDBDictionary)	   // источник db3
			{	
				if (this._iturDictionary.ContainsKey(keyValueItur.Key)	== true) continue;
				yield return keyValueItur.Value;
			}
