...
{
	public class ProductCatalogHashParser : BaseProductCatalogParser, IProductSimpleParser
	{
		public ProductCatalogHashParser(IServiceLocator serviceLocator,
			ILog log)
			...
			Dictionary<ImportProviderParmEnum, object> parms = null)
		{
				...
				countExcludeFirstString))
			{
				...
				//Field4: Item Name								3
				//Field5: Barcode									4
				//Field6: Price Buy (Optional)					5
				//Field7: Quantity In ERP (Optional)		6
				...
				if (record.Length >= 6)
				{
					newMakatProductSimpleString.PriceBuy = record[5].Trim();
				}

				if (withQuantityERP == true)
				{
					string quantityERP = record[6].Trim();
					if (string.IsNullOrWhiteSpace(quantityERP) == false)
					{
						if (record.Length >= 7)
						{
