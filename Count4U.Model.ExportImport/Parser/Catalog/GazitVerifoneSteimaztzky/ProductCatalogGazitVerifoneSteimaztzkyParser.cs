...
{
	public class ProductCatalogGazitVerifoneSteimaztzkyParser : BaseProductCatalogParser, IProductSimpleParser
	{

		public ProductCatalogGazitVerifoneSteimaztzkyParser(IServiceLocator serviceLocator,
			ILog log)
			...
			Dictionary<ImportProviderParmEnum, object> parms = null)
		{
				...
				countExcludeFirstString))
			{
				...
				{
					recordEmpty[j] = recordSplit[j].Trim();
				}
				//Field1: Item Code 										0
				//Field2: Name												1
				...
				string name = recordEmpty[1].ReverseDosHebrew(invertLetter, rt2lf);
				...
				if (string.IsNullOrWhiteSpace(recordEmpty[11]) == true)			 //SubSectionCode	empty
				{
					if (string.IsNullOrWhiteSpace(recordEmpty[13]) == false)		  //SectionCode	  not empty
					{
						newProductSimpleString.SectionCode = recordEmpty[13];//.LeadingZero3(); 
					}
				}
				else
				{
					newProductSimpleString.SectionCode = recordEmpty[11];//.LeadingZero3(); 	//SubSectionCode	  not empty
				}
