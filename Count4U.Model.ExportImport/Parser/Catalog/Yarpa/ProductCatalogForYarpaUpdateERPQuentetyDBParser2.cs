...
{
	public class ProductCatalogForYarpaUpdateERPQuentetyDBParser2 :BaseProductCatalogParser, IProductSimpleParser
	{
		private readonly IProductRepository _productRepository;
		private readonly IImportCatalogADORepository _importCatalogSimpleRepository;

		public ProductCatalogForYarpaUpdateERPQuentetyDBParser2(IProductRepository productRepository,
			ILog log, 
		...
		public IEnumerable<Product> 
			GetProducts(string fromPathFile,
			...
			Dictionary<ImportProviderParmEnum, object> parms = null)
		{
			...
			Dictionary<string, Product> updateQuantityERPDictionaryAdd = new Dictionary<string, Product>();
				...
				countExcludeFirstString))
			{
				...
				String[] record = { "", "0.0", "" };	  //makat quantityERP  //priceSale  priceBuy
				...
				if (record[2] == ".")
				{
					...
					updateQuantityERPDictionaryAdd[makat] = newProductSimple;
				}// end if (record[2] == ".")
			...
			{
				Product updateProductSimple = new Product();		  //get from updateDictionary ProductSimple
				//double quantityERP = 0.0;
				bool exists = updateQuantityERPDictionaryAdd.TryGetValue(productSimple.Makat, out updateProductSimple);	//updateDictionary from input file Dictionary
				if (exists == true)	   //если в файле апдейта есть Макат текущего  ProductSimple, то мы его обновляем
				{
					double balanceQuantityERP = productSimple.BalanceQuantityERP != null ? productSimple.BalanceQuantityERP : 0.0;
					productSimple.BalanceQuantityERP = balanceQuantityERP + updateProductSimple.BalanceQuantityERP * 0.001;
