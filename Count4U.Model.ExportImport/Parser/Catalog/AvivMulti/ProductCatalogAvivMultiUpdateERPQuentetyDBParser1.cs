...
{
	public class ProductCatalogAvivMultiUpdateERPQuentetyDBParser1 :BaseProductCatalogParser, IProductSimpleParser
	{
		private readonly IProductRepository _productRepository;
		private readonly IImportCatalogADORepository _importCatalogSimpleRepository;
	
		public ProductCatalogAvivMultiUpdateERPQuentetyDBParser1(
			...
			Dictionary<ImportProviderParmEnum, object> parms = null)
		{
			...
			//Dictionary<string, ProductSimple> quantityPartialERPDictionary = new Dictionary<string, ProductSimple>();
...
//mlout.txt
   // 67397,     11, 34.56
			// makat,  Quantity Expected,  Price Buy (Optional)
			//Field 3 - Price Buy (optional)
		...
		countExcludeFirstString))
			{
				...
				if (record.Length < 2)
				{
					...
					if (record.Length >= 3)
					{
						bool ret = Double.TryParse(record[2], out quantity);
					}

					//double quantity = 0.0;
					//bool ret = Double.TryParse(record[1], out quantity);
					newProductSimple.PriceBuy = quantity;
