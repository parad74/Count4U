...
{
	public class SupplierDefaultParser : ISupplierParser
	{
		...
		public SupplierDefaultParser(IServiceLocator serviceLocator,
			ILog log)
		{
			...
			Dictionary<ImportProviderParmEnum, object> parms = null)
		{
				...
				countExcludeFirstString))
			{
				...
				if (record.Length >= 3)
				{
					bool yes = Int32.TryParse(record[3].Trim(), out supplierLevel);
				}
				else
				{
					bool yes = Int32.TryParse(record[0].Trim(), out supplierLevel);
				}
				...
				if (this._supplierDictionary.ContainsKey(supplierCode) == false)
				{
					...
					this._supplierDictionary[supplierCode] = new Supplier { SupplierCode = supplierCode, Name = supplierName, Description = "", SupplierLevel = supplierLevel };
				}
