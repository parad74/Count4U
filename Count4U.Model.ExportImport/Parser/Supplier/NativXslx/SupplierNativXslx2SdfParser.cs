...
{
	public class SupplierNativXslx2SdfParser : ISupplierParser
	{
		...
		public SupplierNativXslx2SdfParser(IServiceLocator serviceLocator,
			ILog log)
		{
			...
			Dictionary<ImportProviderParmEnum, object> parms = null)
		{
				...
				countExcludeFirstString))
			{
				...
				if (string.IsNullOrWhiteSpace(record[11].Trim()) == false)
				{
					supplierCode = record[11].Trim();
				}
				

				string supplierName = DomainUnknownName.UnknownSection;
				if (string.IsNullOrWhiteSpace(record[12].Trim()) == false)
				{
					supplierName = record[12].ReverseDosHebrew(invertLetter, rt2lf);
				}
