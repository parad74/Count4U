...
{
	public class ImportTemplateInventoryBulkRepository : BaseCopyBulkRepository, IImportTemplateInventoryRepository
	{
		private ITemplateInventorySQLiteParser _templateInventoryParser;
		private static readonly Logger _logger = LogManager.GetCurrentClassLogger();

		public ImportTemplateInventoryBulkRepository(
			...
			List<string[]> ColumnMappings = null)
		{
			...
			if (countAction == null) throw new ArgumentNullException("ActionUpdateProgress is null");
			 ...
			 this.Log.Add(MessageTypeEnum.TraceRepository, "ImportTemplateInventoryRepository is [ImportTemplateInventoryBulkRepository]");
			...
			//SqlCeTransaction tran = null;
			 ...
			 SqlCeConnection sqlCeConnection = new SqlCeConnection(connectionString);
			  ...
			  try
			  {
				...
				IEnumerable<TemplateInventory> templateInventorys = this._templateInventoryParser.GetTemplateInventorys(fromPathFile,
					encoding, separators, countExcludeFirstString, templateInventoryFromDBDictionary, parms).ToList();
				...
				this.Log.Add(MessageTypeEnum.TraceRepository, String.Format(Localization.Resources.Log_TraceRepositoryResult1035, "ImportTemplateInventoryBulkRepository"));
			
				//Localization.Resources.Log_TraceRepositoryResult1011%"Add {0} {1} in DB [{2}]"
				this.Log.Add(MessageTypeEnum.SimpleTrace, String.Format(Localization.Resources.Log_TraceRepositoryResult1011, count, "TemplateInventory", connectionString));
				//Localization.Resources.Log_TraceRepositoryResult1036%"End process: Commit Data To DB [{0}]"
				this.Log.Add(MessageTypeEnum.TraceRepositoryResult, String.Format(Localization.Resources.Log_TraceRepositoryResult1036, "ImportTemplateInventoryBulkRepository"));
			}

			catch (Exception error)
			{
				_logger.ErrorException("ImportTemplateInventoryBulkRepository", error);
