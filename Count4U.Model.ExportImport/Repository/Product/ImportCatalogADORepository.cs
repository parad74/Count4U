...
{
	public class ImportCatalogADORepository : BaseImportADORepository, IImportCatalogADORepository
	{
		...
		public ImportCatalogADORepository(
			...
			Dictionary<ImportProviderParmEnum, object> parms = null)
		{
			...
			//parms.Add(ImportProviderParmEnum.DBPath, pathDB);
			...
			string sql1 = "INSERT INTO [Product] ([Makat],[ParentMakat],[Name],[MakatOriginal],[Family],[FamilyCode]"
				...
				+ ",[SectionName],[SubSectionCode], [SubSectionName],[SupplierName], [ItemType]) "
			...
			try
			{
				...
				cmd.Parameters.Add(new SqlCeParameter(
				"@SubSectionCode", SqlDbType.NVarChar, 50));
				cmd.Parameters.Add(new SqlCeParameter(
				"@SubSectionName", SqlDbType.NVarChar, 100));
				cmd.Parameters.Add(new SqlCeParameter(
				"@SupplierName", SqlDbType.NVarChar, 100));
				cmd.Parameters.Add(new SqlCeParameter(
				"@ItemType", SqlDbType.NVarChar, 50));
				...
				if (importType.Contains(ImportDomainEnum.ImportCatalog) == true)
				{
						...
						encoding, separators, countExcludeFirstString, productMakatDictionary, importType, parms))
					{
						...
						cmd.Parameters["@SupplierName"].Value = product.SupplierName;
						...
						cmd.Parameters["@SectionName"].Value = product.SectionName;
						cmd.Parameters["@SubSectionCode"].Value = product.SubSectionCode;
						cmd.Parameters["@SubSectionName"].Value = product.SubSectionName;
						cmd.Parameters["@SectionName"].Value = product.SectionName;
						cmd.Parameters["@ItemType"].Value = product.ItemType;
		...
		public void ClearProductsMakatOnly(string pathDB)
		{
			...
			string sql1 = "DELETE FROM  [Product] WHERE ([TypeCode] = " + @"'" + makatType + @"'" + ")";
			SqlCeTransaction tran = null;
			string connectionString = this.BuildADOConnectionStringBySubFolder(pathDB);
			SqlCeConnection sqlCeConnection = new SqlCeConnection(connectionString);
