...
namespace Count4U.Model.Count4U
{
	public class ImportPreviousInventorySQLiteADORepository : BaseImportADORepository, IImportPreviousInventorysSQLiteADORepository
	{
  	...
  	//	private readonly ILocationRepository _locationRepository;
		...
		public void InsertPreviousInventorys(string fromPathFile, 	 //GetDbPath		Count4U
			...
			Dictionary<ImportProviderParmEnum, object> parms = null)
		{
			...
			//DbConnection previousDBConnection = new SqlCeConnection();
			//previousDBConnection.ConnectionString
			//PreviousDBContext db = new PreviousDBContext(previousDBConnection);
			...
			if (importType.Contains(ImportDomainEnum.ExistPreviousInventory) == true)
			{
				...
				previousInventoryFromDBDictionary = _previousInventoryEFRepository.GetDictionaryPreviousInventorysByUid(fromPathFile);
			}
	 ...
	 //	  ,<DateCreated, text,>);
      ...
      " VALUES(" +
			...
			using (SQLiteConnection sqliteConnection = new SQLiteConnection())
			{
				...
				try
				{
					...
					using (SQLiteCommand cmd = new SQLiteCommand(sql1, sqliteConnection, tran))
					{
						...
						this.Log.Add(MessageTypeEnum.TraceRepository, String.Format(Localization.Resources.Log_TraceRepositoryResult1003, "PreviousInventory", toPathDB3));
					...
					this.Log.Add(MessageTypeEnum.SimpleTrace, String.Format(Localization.Resources.Log_TraceRepositoryResult1011, k, "PreviousInventory", toPathDB3));
		...
		public void VacuumPreviousInventory(string pathDB3)
		{
			if (File.Exists(pathDB3) == false) return;
			string sql1 = "VACUUM  [PreviousInventory]";
			SQLiteTransaction tran = null;
			...
			using (SQLiteConnection sqliteConnection = new SQLiteConnection())
			{
				...
				try
				{
					...
					using (SQLiteCommand cmd = new SQLiteCommand(sql1, sqliteConnection, tran))
					{
						this.Log.Add(MessageTypeEnum.TraceRepository, String.Format("VACUUM PreviousInventory in [{0}] ", pathDB3));
						cmd.ExecuteNonQuery();
					}
					this.Log.Add(MessageTypeEnum.TraceRepository, String.Format("VACUUM  [{0}] ", "PreviousInventory"));
				}
				catch (Exception error)
				{
					_logger.ErrorException("VacuumPreviousInventory", error);
					this.Log.Add(MessageTypeEnum.ErrorDB, error.Message + " : " + error.StackTrace);
				}
				finally
				{
