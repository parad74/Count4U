...
{
	public class ImportIturMerkavaXslxProvider : BaseProvider, IImportProvider
	{
		private readonly IImportIturRepository _importIturADORepository;

		public ImportIturMerkavaXslxProvider(IImportIturRepository importIturADORepository,
		...
		{
			//this.InitConfig();
			//this.InitFromUserParms();

			this.FillInfoLog(this.FromPathFile, this.GetType().Name, this._importTypes);
			if (this.IsEmptyPath(this.FromPathFile) == true) return;
			if (this.IsEmptyPath(this.ToPathDB) == true) return;
			this._importIturADORepository.InsertIturs(this.FromPathFile, this.ToPathDB, IturParserEnum.IturMerkavaXslxParser,
