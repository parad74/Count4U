...
namespace Count4U.ImportPdaMISAndForwardAdapter
{
    public class ImportPdaMISAndForwardAdapterViewModel : TemplateAdapterFileFolderViewModel, IImportPdaAdapter
    {
		...
		public ImportPdaMISAndForwardAdapterViewModel(
...
//MISCommunicatorPath = C:\MIS\MISCommunicator
		...
		public override void Import()
		{
			...
			//bool copyByCodeInventor = this._userSettingsManager.CopyByCodeInventorGet();
			...
			try
			{
				...
				//IExportProvider providerForward = base.ServiceLocator.GetInstance<IExportProvider>(ImportProviderEnum.ForwardInventProductMalamFileProvider.ToString());
				IExportProvider providerForward = base.ServiceLocator.GetInstance<IExportProvider>(ImportProviderEnum.ForwardInventProductMalamDataSetProvider.ToString());

				providerForward.FromPathDB = base.GetDbPath; // не должно поиспользоваться 

				IImportProvider provider = this.GetProviderInstance(ImportProviderEnum.ImportInventProductMisADOProvider);
				...
				if (base.IsSingleFile == true)
				{
					...
					string currenfolder = System.IO.Path.GetDirectoryName(this.Path) + @"\Forward";
					if (Directory.Exists(currenfolder) == false) Directory.CreateDirectory(currenfolder);
					string tofilePath = System.IO.Path.GetDirectoryName(this.Path) + @"\Forward\" + System.IO.Path.GetFileNameWithoutExtension(this.Path) + ".xml";

					providerForward.Parms.Clear();
					provider.Parms.AddCancellationUpdate(base.UpdateProgress, base.CancellationToken);
					//provider.Parms[ImportProviderParmEnum.NewDocumentCode] = newDocumentCode;
					providerForward.Parms[ImportProviderParmEnum.NewSessionCode] = newSessionCode;
					providerForward.Parms[ImportProviderParmEnum.DBPath] = base.GetDbPath;

					providerForward.Parms[ImportProviderParmEnum.SourcePath] = this.Path;
					providerForward.Parms[ImportProviderParmEnum.DestinationPath] = tofilePath;
					StepCurrent = 2;
					providerForward.Export();
				...
				else
				{
					...
					while (true)
					{
						...
						string currenfolder = this.Path + @"\Forward";
						if (Directory.Exists(currenfolder) == false) Directory.CreateDirectory(currenfolder);
						...
						foreach (string filePath in files)
						{
							...
							providerForward.Parms[ImportProviderParmEnum.SourcePath] = finalPath;
							string tofilePath = System.IO.Path.GetDirectoryName(finalPath) + @"\Forward\" + System.IO.Path.GetFileNameWithoutExtension(finalPath) + ".xml";
							providerForward.Parms[ImportProviderParmEnum.DestinationPath] = tofilePath;
							providerForward.Export();
				...
				foreach (var documentCode in currentDocumentCodeList)
				{
					...
					base.LogImport.Add(MessageTypeEnum.EndTrace, String.Format("Document {0} was print", documentCode));
				}
