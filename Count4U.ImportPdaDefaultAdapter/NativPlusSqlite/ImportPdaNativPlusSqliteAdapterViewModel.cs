...
namespace Count4U.ImportPdaNativPlusSqliteAdapter
{
    public class ImportPdaNativPlusSqliteAdapterViewModel : TemplateAdapterFileFolderViewModel, IImportPdaAdapter
    {
		...
		public ImportPdaNativPlusSqliteAdapterViewModel(
        ...
        public override void InitFromIni()
        {
			...
			Dictionary<ImportProviderParmEnum, string> iniData1 = base.GetIniData("ImportPdaNativPlusSqliteAdapter");
		...
		public override void Import()
		{
			...
			try
			{
				...
				//IturUpdateNativSqlite2SdfParser																																					  //this faster
				IImportProvider providerItur = this.GetProviderInstance(ImportProviderEnum.ImportLocationUpdateNativSqlite2SdfProvider);//(ImportProviderEnum.ImportLocationUpdateNativSimpleSqlite2SdfProvider);
				...
				IImportProvider providerInventProduct = this.GetProviderInstance(ImportProviderEnum.ImportInventProductNativPlusSqlite2SdfProvider);
				...
				else
				{
					...
					while (true)
					{
						...
						List<string> fileszip = new List<string>();
						foreach (FileInfo fi in sourceFiles)
						{
							...
							{
								fileszip.Add(fileName);
							}
						}

						foreach (string fz in fileszip)
						{
							//unzip  FROM inData\zip TO  Count4U inData
							this._zip.ReadDb3FromZipFile(this.Path + @"\"+ fz, this.Path);
						}

						sourceFiles = dir.GetFiles().OrderBy(f => f.LastWriteTime).ToArray();
						foreach (FileInfo fi in sourceFiles)
						{
							string fileName = fi.Name;
						...
						base.StepTotal = files.Count * 5 + 2;
						base.StepCurrent = 0;
						//=====================	  Temporary 
						...
						foreach (string filePath in files)
						{
							...
							providerItur.Parms.AddCancellationUpdate(base.UpdateProgress, base.CancellationToken);
							providerItur.Parms[ImportProviderParmEnum.DBPath] = base.GetDbPath;
							providerItur.Parms.AddInventProductAdvancedField(this.InventProductAdvancedDBFieldName);
							string finalPath = System.IO.Path.Combine(this.Path, filePath);
