...
{
    public class ImportPdaWarehouseAdapterViewModel : TemplateAdapterFileFolderViewModel, IImportPdaAdapter
    {
		...
		public ImportPdaWarehouseAdapterViewModel(
		...
		public string GetIturCodeByDefault
		{
			get
			{
				string iturCodeByDefault = "2016";
				IIturRepository iturRepository = base.ServiceLocator.GetInstance<IIturRepository>();
				IContextCBIRepository contextCBIRepository = base.ServiceLocator.GetInstance<IContextCBIRepository>();
				string customerDBPath = contextCBIRepository.GetDBPath(base.CurrentCustomer);
				List<string> iturCodeList = iturRepository.GetIturCodeList(customerDBPath);
				if (iturCodeList.Count >0 )	   iturCodeByDefault	= iturCodeList[0];
  				return iturCodeByDefault;
			}
		}
        ...
        {
			IImportProvider provider = this.GetProviderInstance(ImportProviderEnum.ImportInventProductWarehouseADOProvider);
            ...
            if (base.IsSingleFile == true)
            {
				...
				provider.Parms[ImportProviderParmEnum.IturCode] = GetIturCodeByDefault;
