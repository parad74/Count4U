...
namespace Count4U.ImportPdaDB3Adapter
{
    public class ImportPdaDB3AdapterViewModel : TemplateAdapterFileFolderViewModel, IImportPdaAdapter
    {
		...
		public ImportPdaDB3AdapterViewModel(
...
//MISCommunicatorPath = C:\MIS\MISCommunicator
			...
			Dictionary<ImportProviderParmEnum, string> iniData1 = base.GetIniData("ImportPdaDB3Adapter");
			this.InventProductAdvancedDBFieldName = iniData1.SetValueName(this.InventProductAdvancedDBFieldName);
		//	InventProductAdvancedFieldName SetValue(this Dictionary<ImportProviderParmEnum, string> iniParms,
        ...
        public override void Import()
        {
		   ...
		   try{
			...
			IImportProvider provider = this.GetProviderInstance(ImportProviderEnum.ImportInventProductDB3Provider);
            ...
            if (base.IsSingleFile == true)
            {
				...
				provider.Parms.AddInventProductAdvancedField(this.InventProductAdvancedDBFieldName);
				...
				{
					provider.Parms[ImportProviderParmEnum.FileDB3] = "1";
					StepCurrent = 1;
					provider.Import();
				}
				else
            ...
            else
            {
                ...
                while (true)
                {
					...
					foreach (string filePath in getFiles)
					{
						...
						if (System.IO.Path.GetExtension(filePath) == ".db3")
						{
							files.Add(filePath);
						}
						else
