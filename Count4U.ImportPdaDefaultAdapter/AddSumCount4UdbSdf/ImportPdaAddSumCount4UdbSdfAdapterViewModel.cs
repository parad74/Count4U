...
namespace Count4U.ImportPdaAddSumCount4UdbSdfAdapter
{
	public class ImportPdaAddSumCount4UdbSdfAdapterViewModel : TemplateAdapterFileFolderViewModel, IImportPdaAdapter
    {
		...
		public ImportPdaAddSumCount4UdbSdfAdapterViewModel(
		...
		{							
			...
			IImportProvider providerUpdate2Sum = this.GetProviderInstance(ImportProviderEnum.ImportInventProductUpdate2SumByIturMakatDbBulkProvider);
			providerUpdate2Sum.ProviderEncoding = base.Encoding;
			...
			IImportProvider providerInventProduct = this.GetProviderInstance(ImportProviderEnum.ImportInventProductFromDbBulkProvider);
            providerInventProduct.ProviderEncoding = base.Encoding;
                ...
                while (true)
                {
                    ...
                    base.Session++;
					base.StepTotal = itemsCheck.Count * 5;
                    base.StepCurrent = 0;
					...
					{
						base.StepCurrent++;
						providerUpdate2Sum.Parms.Clear();
						providerUpdate2Sum.Parms.AddCancellationUpdate(base.UpdateProgress, base.CancellationToken);
						providerUpdate2Sum.Parms[ImportProviderParmEnum.NewSessionCode] = newSessionCode;
						providerUpdate2Sum.Parms[ImportProviderParmEnum.DBPath] = item.Path; 
						providerUpdate2Sum.FromPathFile = item.Path;
						providerUpdate2Sum.ToPathDB = item.Path;		
						providerUpdate2Sum.Import();

						IDocumentHeaderRepository docRepository = base.ServiceLocator.GetInstance<IDocumentHeaderRepository>();
						docRepository.DeleteAllDocumentsWithoutAnyInventProduct(item.Path);
					...
					foreach (FileItemViewModel item in itemsCheck)
					{
						...
						providerInventProduct.Parms[ImportProviderParmEnum.DBPath] = getDbPath;		//base.GetDbPath;
						providerInventProduct.FromPathFile = item.Path;
						providerInventProduct.ToPathDB = getDbPath;		//  base.GetDbPath;
