...
namespace Count4U.ImportPdaMinusByMakatCount4UdbSdfAdapter
{
    public class ImportPdaMinusByMakatCount4UdbSdfAdapterViewModel : TemplateAdapterFileFolderViewModel, IImportPdaAdapter
    {
		...
		public ImportPdaMinusByMakatCount4UdbSdfAdapterViewModel(
        ...
        public override void InitFromIni()
        {
			...
			this.Path = System.IO.Path.GetFullPath(base.GetImportPath().Trim('\\') + @"\" + this._fileName) + @"\Log";
			this.Build();
			this.RaisePropertyChanged(() => this.Items);
		...
		//		{
		//			DBSettings dbSettings = base.ServiceLocator.GetInstance<DBSettings>();
		...
		//	Inventor sourceInventorDb = SelectedItem.InventorObject;
			...
			IImportProvider provider3 = this.GetProviderInstance(ImportProviderEnum.ImportInventProductMinusByMakatFromDbBulkProvider);
			...
			while (true)
			{
				...
				//{
					...
					provider3.FromPathFile = SelectedItem.Path;
					provider3.Import();

					base.LogImport.Add(MessageTypeEnum.EndTrace, String.Format("Path : {0}", SelectedItem.Path));
