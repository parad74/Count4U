...
{
	public static class String_ToRequest09
	{
		// пришла строка с liber8 - надо распарсить  в объект (dataContract) и передать дальше на Client_Host
		public static Request09 StringToRequest09(this string entity)
		{
			...
			if (codeOperationEnum == CodeOperationEnum.CodeOperation09)
			{
				Sip2BaseData response = String_ToRequestBase.StringToRequestBase(entity, request, codeOperationEnum);
				return response as Request09;
			}
			else
			{
				request.Error = "Code operation is expected " + (int)CodeOperationEnum.CodeOperation09;
				return request;
			}
		}
	}
