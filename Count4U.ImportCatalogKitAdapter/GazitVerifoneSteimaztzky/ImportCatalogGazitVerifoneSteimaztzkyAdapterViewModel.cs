...
namespace Count4U.ImportCatalogGazitVerifoneSteimaztzkyAdapter
{
    public class ImportCatalogGazitVerifoneSteimaztzkyAdapterViewModel : TemplateAdapterTwoFilesViewModel
    {
		...
		public ImportCatalogGazitVerifoneSteimaztzkyAdapterViewModel(IServiceLocator serviceLocator,
		...
		private int GetStep()
		{
			...
			if (this.ImportSection == true) step = step + 2;
			return step;
		}
        ...
        public override void InitDefault(CBIState state = null)
        {
			...
			//	inventorDate = dt.ToString("yyyyMMdd");
			//}
			...
			this._fileName1 = "steimatzky_catalog.txt";
			this._fileName2 = String.Format("Store_{0}_{1}.txt", this._branchErpCode, currentDate);	   	 //Store_XXX_YYYMMDD.txt 
        ...
        {
			if (this.WithQuantityErp == true)
			{	 //Store_XXX_YYYMMDD.txt 	
				string currentDate = "_" + DateTime.Now.ToString("yyyyMMdd");
				string filename = @"Store_(.+)" + currentDate + @"\.txt";
				return base.ContinueAfterBranchERPWarning(base.Path2, filename);
			}
			else
			{
				return true;
        ...
        public override void Import()
        {
			...
			IImportProvider provider = this.GetProviderInstance(ImportProviderEnum.ImportCatalogGazitVerifoneSteimaztzkyProvider);
			...
			IImportProvider provider1 = this.GetProviderInstance(ImportProviderEnum.ImportCatalogGazitVerifoneSteimaztzkyProvider1);
			...
			if (this.WithQuantityErp == true)
			{
				IImportProvider provider3 = this.GetProviderInstance(ImportProviderEnum.ImportCatalogGazitVerifoneSteimaztzkyUpdateERPQuentetyProvider);
			...
			{
				IImportProvider provider4 = this.GetProviderInstance(ImportProviderEnum.ImportSectionGazitVerifoneSteimaztzkyProvider);
				...
				IImportProvider provider5 = this.GetProviderInstance(ImportProviderEnum.ImportSectionGazitVerifoneSteimaztzkyProvider1);
        ...
        public override void Clear()
        {
			...
			{
				IImportProvider provider3 = this.GetProviderInstance(ImportProviderEnum.ImportSectionGazitVerifoneSteimaztzkyProvider);
				provider3.ToPathDB = base.GetDbPath;
				provider3.Clear();
			}
