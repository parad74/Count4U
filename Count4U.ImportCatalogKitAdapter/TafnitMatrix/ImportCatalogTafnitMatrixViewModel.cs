...
namespace Count4U.ImportCatalogKitAdapter.TafnitMatrix
{
    public class ImportCatalogTafnitMatrixViewModel : TemplateAdapterOneFileViewModel
    {
		...
		public ImportCatalogTafnitMatrixViewModel(IServiceLocator serviceLocator,
		...
		public bool WithQuantityErp
		{
			...
			set
			{
				...
				//	base.StepTotal = 2;
				//}
				//if (this.ImportBarcodeWithSpace == true) base.StepTotal = base.StepTotal + 1;
        ...
        public override void InitDefault(CBIState state = null)
        {
            ...
            if (base.CurrentBranch != null)
            {
				...
				if (string.IsNullOrWhiteSpace(base.CurrentBranch.BranchCodeERP) == false)
				{
					branchCodeERP = base.CurrentBranch.BranchCodeERP.TrimStart('0');
					if (string.IsNullOrWhiteSpace(branchCodeERP) == true) 
					{
						branchCodeERP = "00"; 
					}
				}
            ...
            //else  if (base.CurrentCustomer != null)
            //{
            //     branchCode = base.CurrentCustomer.Code;
            //}
         

            //init GUI
		   this.PathFilter = "*.txt|*.txt|All files (*.*)|*.*";
		   this._fileName = branchCodeERP + ".txt"; //(XX - Branch code without leading zero's); ????
		  // this._fileName = "ITM.MSF";
            base.IsInvertLetters = true;
			base.IsInvertWords = true;
            base.Encoding = System.Text.Encoding.GetEncoding(1255);

			this._importBarcodeWithSpace = false;
        ...
        {
			Dictionary<ImportProviderParmEnum, string> iniData = base.GetIniData(String.Empty, base.GetPathToIniFile("Count4U.ImportCatalogTafnitMatrixAdapter.ini"));
            //init GUI
		...
		protected override bool PreImportCheck()
		{
			...
			return base.ContinueAfterBranchERPWarning(base.Path, @"(.+)\.txt", branchCodeERP); 
        ...
        public override void Import()
        {
			...
			IImportProvider provider = this.GetProviderInstance(ImportProviderEnum.ImportCatalogTafnitMatrixProvider);
			...
			IImportProvider provider1 = this.GetProviderInstance(ImportProviderEnum.ImportCatalogTafnitMatrixProvider1);
			...
			IImportProvider provider2 = this.GetProviderInstance(ImportProviderEnum.ImportCatalogTafnitMatrixProvider2);
        ...
        public override void Clear()
        {
			...
			IImportProvider provider1 = this.GetProviderInstance(ImportProviderEnum.ImportSectionTafnitMatrixProvider);
			provider1.ToPathDB = base.GetDbPath;
			provider1.Clear();

			IImportProvider provider2 = this.GetProviderInstance(ImportProviderEnum.ImportFamilyTafnitMatrixProvider);
			provider2.ToPathDB = base.GetDbPath;
			provider2.Clear();
