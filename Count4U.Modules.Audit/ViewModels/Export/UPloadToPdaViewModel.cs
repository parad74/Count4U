...
{
    public class UploadToPdaViewModel : CBIContextBaseViewModel
    {
            ...
            : base(contextCbiRepository)
        {
			...
			this._uploadButtonText = Localization.Resources.View_UploadToPda_btnUpload;
        ...
        public bool IsUploading
        {
            ...
            set
            {
				...
				if (this._isUploading == true)
					this._uploadButtonText = Localization.Resources.View_UploadToPda_btnStop;
                else
					this._uploadButtonText = Localization.Resources.View_UploadToPda_btnUpload;

                RaisePropertyChanged(() => UploadButtonText);
            }
        }
        ...
        {
			this._uploadButtonText = Localization.Resources.View_UploadToPda_btnUpload;
			this.RaisePropertyChanged(() => this.UploadButtonText);
			...
			try
			{
				...
				if (abort == false)
				{
					...
					this._wrapperMultiRepository.AbortThreadAllThreads(wrapperMultis, from + "- >Done", this._userSettingsManager.UploadWakeupTimeGet());
				}
		...
		private void Upload()
		{
			...
			using (new CursorWait())
			{
				this.AssignTerminalID("Upload", false);
			}
        ...
        private void AssignTerminalID( string from, bool abortThreadAfter = false)
        {
            try
            {
				...
				//List<Multi> multis = this._items.Select(x => x.UploadWrapperMulti.Multi).ToList(); //@@
				List<IWrapperMulti> wrapperMultis = this._items.Select(x => x.wrapperMulti).ToList();
				//List<IUploadPdaUnit> units  = this._items.Select(x=> x.UploadUnit).ToList(); //было
				...
				//--------------------GetTerminalID and set in GUI
					...
					for (int i = 0; i < this._items.Count; i++)
					{
						...
						//} //lock
						//------------
						//terminalId = this._items[i].UploadWrapperMulti.Multi.TerminalID; //add // @@
				...
				int uploadWakeupTimeOutFinish = _userSettingsManager.UploadWakeupTimeGet() * 300;
				...
				{
					//List<IWrapperMulti> wrapperMultis = this._items.Select(x => x.UploadWrapperMulti).ToList(); // см выше дублированно @@
				...
				else  // или готовим к uploade
				{
					...
					//List<Multi> multisNotStart = this._items.Where(x => x.Device == "not started").Select(x => x.UploadWrapperMulti.Multi).ToList(); //@@
					Thread.Sleep(uploadWakeupTimeOutFinish);
		...
		//private void AbortThreads(string from)
		//{
		//	_logger.Info("AbortThreadAllThreads start from " + from);
		//	int uploadWakeupTimeOutFinish1 = _userSettingsManager.UploadWakeupTimeGet() * 100;
		//	Thread.Sleep(uploadWakeupTimeOutFinish1);

		//	List<IUploadPdaUnit> units = this._items.Select(x => x.UploadUnit).ToList();
		//	this._uploadPdaRepository.AbortThreadAllThreads(units, "AbortThreads::AbortThreadAllThreads" + from);

		//	_logger.Info("AbortThreadAllThreads finish " + from);
		//	int uploadWakeupTimeOutFinish = _userSettingsManager.UploadWakeupTimeGet() * 100;
		//	Thread.Sleep(uploadWakeupTimeOutFinish);
		//	this._isDesposed = true;
		//}
		...
		void UploadPdaUnit_StateChanged(IWrapperMulti uploadPdaWrapperMulti, UploadPdaMPStage stage, int state, string text, int current) // идет типа от EventMPState(this, MPStage.Download, (int)MPError.FileStream, file, file_size); в multi
        {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (fileIndex >= 0)
                        {
                            ...
                            UpdateTotalProgress();
                            //всего <= закочено && последний файл из списка файлов
							//if (current <= state && fileIndex + 1 == item._uploadingFiles.Count)			//state/current  //закочено/всего //1075/1185
							//{
							//	item.Finish();
							//}
