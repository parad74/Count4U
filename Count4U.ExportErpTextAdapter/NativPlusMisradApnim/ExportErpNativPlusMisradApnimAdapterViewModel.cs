...
namespace Count4U.ExportErpTemplateAdapter.NativPlusMisradApnim
{
    public class ExportErpNativPlusMisradApnimAdapterViewModel : ExportErpMakatViewModel
    {
		...
		public ExportErpNativPlusMisradApnimAdapterViewModel(IContextCBIRepository contextCbiRepository,
                                               ILog logImport,
        ...
        protected override void RunExportInner(ExportErpCommandInfo info)
        {
            ...
            //FILE1
//Aman_CurrentDate&Time.xlsxuse the current date and time. not the inventory date
			string fileName1 = String.Format("Aman_Current_{0}", inventorDate3); //result file only name without extension            
			...
			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductNativPlusMisradApnim.ToString());
