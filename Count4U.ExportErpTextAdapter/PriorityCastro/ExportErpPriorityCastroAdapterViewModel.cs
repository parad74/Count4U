...
{
    public class ExportErpPriorityCastroAdapterViewModel : ExportErpMakatViewModel
    {

		public ExportErpPriorityCastroAdapterViewModel(IContextCBIRepository contextCbiRepository,
            ILog logImport,
        ...
        {
			return Path.Combine(base.GetModulesFolderPath(), "ExportErpPriorityCastroAdapter.ini");            
        }
        ...
        protected override void RunExportInner(ExportErpCommandInfo info)
        {
			...
			string fileName = String.Format("{0}", branchCodeErp); //result file only name without extension    
			...
			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductPriorityCastroERPFileProvider.ToString());
			...
			IExportERPProvider provider1 = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductPriorityCastroERPFileProvider1.ToString());
