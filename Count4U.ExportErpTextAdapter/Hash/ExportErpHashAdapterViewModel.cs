...
namespace Count4U.ExportErpTextAdapter.Hash
{
    public class ExportErpHashAdapterViewModel: ExportErpModuleBaseViewModel
    {
		public ExportErpHashAdapterViewModel(IContextCBIRepository contextCbiRepository,
            ILog logImport,
        ...
        {
			return Path.Combine(base.GetModulesFolderPath(), "ExportErpHashAdapter.ini");
        }
        ...
        {         
            ...
            if (base.CurrentInventor != null)
            {
                ...
                inventorDate = dt.ToString("ddMMyy");
				inventorDateFile1 = dt.ToString("ddMMyyyy");
            }

			string fileName = String.Format("IMOVEIN_{0}_{1}", branchCodeErp, inventorDateFile1); //result file only name without extension   //"IMOVEIN_" & CodeERP & "_" (InventorDate)DDMMYYYY & ".DAT"
			...
			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductHashERPFileProvider.ToString());
			...
			IExportERPProvider provider1 = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductHashERPFileProvider1.ToString());
