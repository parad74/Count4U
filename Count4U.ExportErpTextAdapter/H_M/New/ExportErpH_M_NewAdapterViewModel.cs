...
{
    public class ExportErpH_M_NewAdapterViewModel : ExportErpMakatViewModel
    {

		public ExportErpH_M_NewAdapterViewModel(IContextCBIRepository contextCbiRepository,
            ILog logImport,
        ...
        {
			return Path.Combine(base.GetModulesFolderPath(), "ExportErpН_М_NewAdapter.ini");            
        }
        ...
        protected override void RunExportInner(ExportErpCommandInfo info)
        {
			...
			string fileName = String.Format("INV_{0}_{1}_", branchCodeErp, inventorDateFile1); //result file only name without extension "LadyComfot_" & CodeERP & "_" (InventorDate)DDMMYYYY & ".CSV   
			...
			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductH_M_NewERPFileProvider.ToString());
