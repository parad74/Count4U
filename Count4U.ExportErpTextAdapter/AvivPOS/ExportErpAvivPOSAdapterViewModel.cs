...
namespace Count4U.ExportErpTextAdapter.AvivPOS
{
    public class ExportErpAvivPOSAdapterViewModel : ExportErpMakatViewModel
    {

        public ExportErpAvivPOSAdapterViewModel(IContextCBIRepository contextCbiRepository,
            ILog logImport,
        ...
        {
            return Path.Combine(base.GetModulesFolderPath(), "ExportErpAvivPOSAdapter.ini");
        }
        ...
        protected override void RunExportInner(ExportErpCommandInfo info)
        {
            ...
            string fileName = String.Format("COUNT_{0}_{1}", branchCodeErp, inventorDate); //result file only name without extension    
			...
			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductAvivPOSERPFileProvider.ToString());
