...
{
    public class ExportErpPriorityKedsShowRoomAdapterViewModel : ExportErpMakatViewModel
    {
		public ExportErpPriorityKedsShowRoomAdapterViewModel(IContextCBIRepository contextCbiRepository,
                                               ILog logImport,
        ...
        {
			return Path.Combine(base.GetModulesFolderPath(), "ExportErpPriorityKedsShowRoomAdapter.ini");
        }
        ...
        protected override void RunExportInner(ExportErpCommandInfo info)
        {
            ...
            if (base.CurrentInventor != null)
            {
				...
				inventorDate1 = dt.ToString("dd") + @"/" + dt.ToString("MM") + @"/" + dt.ToString("yy");
            }

            //FILE1
			string fileName1 = String.Format("MATRIX_COUNT{0}_{1}", branchCodeErp, inventorDate); //result file only name without extension            
			...
			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductPriorityKedsShowRoomERPFileProvider.ToString());
			...
			//IExportERPProvider provider1 = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductPriorityKedsShowRoomERPFileProvider1.ToString());
