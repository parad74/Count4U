...
{
    public class ExportErpFRSVisionMirkamAdapterViewModel : ExportErpMakatViewModel
    {

		public ExportErpFRSVisionMirkamAdapterViewModel(IContextCBIRepository contextCbiRepository,
            ILog logImport,
        ...
        {
			return Path.Combine(base.GetModulesFolderPath(), "ExportErpFRSVisionMirkamAdapter.ini");            
        }
        ...
        protected override void RunExportInner(ExportErpCommandInfo info)
        {
			...
			string fileName = String.Format("G{0}{1}", branchCodeErp, inventorDateFile); //result file only name without extension    
			...
			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductFRSVisionMirkamERPFileProvider.ToString());
			...
			IExportERPProvider provider1 = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductFRSVisionMirkamERPFileProvider1.ToString());
