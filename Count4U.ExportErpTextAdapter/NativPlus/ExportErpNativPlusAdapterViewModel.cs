...
namespace Count4U.ExportErpTemplateAdapter.NativPlus
{
    public class ExportErpNativPlusAdapterViewModel : ExportErpMakatViewModel
    {
		...
		public ExportErpNativPlusAdapterViewModel(IContextCBIRepository contextCbiRepository,
                                               ILog logImport,
		...
		public DateTime ModifyDate
		{
			...
			set
			{
				...
				RaisePropertyChanged(() => this.ModifyDate);
			}
		}

		private bool _onModifyDate;
	
		[NotInludeAttribute]
		public bool OnModifyDate
		{
			get { return this._onModifyDate; }
			set
			{
				this._onModifyDate = value;
				RaisePropertyChanged(() => this.OnModifyDate);
			}
		}
...
//2. Add current date to file name (_08-11-2017)
			...
			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductNativPlusProvider1.ToString());
			...
			//if (OnModifyDate == true)
			//{
			//	provider.Export();
			//}

			provider.Export(info.IsFull,  info.IsFilterByLocations, info.LocationCodeList, info.IsFilterByIturs, info.IturCodeList, OnModifyDate, ModifyDate);
			...
			//}
			if (info.IsFull == true && info.IsFilterByIturs == false && info.IsFilterByLocations == false && OnModifyDate == false)
			{
				//provider_Q.Export(info.IsFull, info.IsFilterByLocations, info.LocationCodeList, info.IsFilterByIturs, info.IturCodeList);
				provider_Q.Export();
			}
			...
			{
				provider_Q.Export(info.IsFull, info.IsFilterByLocations, info.LocationCodeList, info.IsFilterByIturs, info.IturCodeList, OnModifyDate, ModifyDate);
			}
			...
			{
				provider_SN.Export(info.IsFull, info.IsFilterByLocations, info.LocationCodeList, info.IsFilterByIturs, info.IturCodeList, OnModifyDate, ModifyDate);
			}
			...
			{
				provider2.Export(info.IsFull, info.IsFilterByLocations, info.LocationCodeList, info.IsFilterByIturs, info.IturCodeList, OnModifyDate, ModifyDate);
			}
