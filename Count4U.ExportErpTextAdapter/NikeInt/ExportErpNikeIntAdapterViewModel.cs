...
{
    public class ExportErpNikeIntAdapterViewModel : ExportErpMakatViewModel
    {

		public ExportErpNikeIntAdapterViewModel(IContextCBIRepository contextCbiRepository,
                                               ILog logImport,
        ...
        {
			return Path.Combine(base.GetModulesFolderPath(), "ExportErpNikeIntAdapter.ini");
        }
        ...
        protected override void RunExportInner(ExportErpCommandInfo info)
        {
            ...
            if (base.CurrentInventor != null)
            {
				...
				inventorDate1 = dt.ToString("yy") + dt.ToString("MM")  + dt.ToString("dd");//YYMMDD
            }

            //FILE1
			//STOCK_000XXX.INV  (XXX – ERP Code Branch)
			string fileName1 = String.Format("Stock_000{0}", branchCodeErp); //result file only name without extension            
			string fileNameWithExtension1 = String.Format("{0}.inv", fileName1);
			string fullPath1 = Path.Combine(base.PathToExportErp, fileNameWithExtension1);

			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductNikeIntERPFileProvider.ToString());
			...
			provider.Parms[ImportProviderParmEnum.FileXlsx] = base.ExportToExcel ? "1" : String.Empty;					//test excel
			...
			IExportERPProvider provider1 = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductNikeIntERPFileProvider1.ToString());
			...
			provider1.Parms[ImportProviderParmEnum.FileXlsx] =  base.ExportToExcel ? "1" : String.Empty;					//test excel
