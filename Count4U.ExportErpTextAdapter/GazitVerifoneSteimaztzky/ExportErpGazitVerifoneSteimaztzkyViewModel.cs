...
{
    public class ExportErpGazitVerifoneSteimaztzkyAdapterViewModel : ExportErpMakatViewModel
    {

		public ExportErpGazitVerifoneSteimaztzkyAdapterViewModel(IContextCBIRepository contextCbiRepository,
                                               ILog logImport,
        ...
        {
			return Path.Combine(base.GetModulesFolderPath(), "ExportErpGazitVerifoneSteimaztzkyAdapter.ini");
        }
        ...
        protected override void RunExportInner(ExportErpCommandInfo info)
        {
            ...
            if (base.CurrentInventor != null)
            {
				...
				inventorDate2 = dt.ToString("dd") + @"/" + dt.ToString("MM") + @"/" + dt.ToString("yyyy");	   //"DD/MM/YYYY"
            }

            //FILE1
			//SYYYYMMDD.XXX 
			string fileName1 = "S" + inventorDate1; //result file only name without extension            
			string fileNameWithExtension1 = fileName1 + "." + branchCodeErp;
            string fullPath1 = Path.Combine(base.PathToExportErp, fileNameWithExtension1);

			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductGazitVerifoneSteimaztzkyERPFileProvider.ToString());
			...
			provider.Parms[ImportProviderParmEnum.InventorDate] = inventorDate2;
			...
			IExportERPProvider provider1 = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductGazitVerifoneSteimaztzkyERPFileProvider1.ToString());
