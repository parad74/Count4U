...
{
    public class ExportErpOrenMutagimAdapterViewModel : ExportErpMakatViewModel
    {

		public ExportErpOrenMutagimAdapterViewModel(IContextCBIRepository contextCbiRepository,
            ILog logImport,
        ...
        {
			return Path.Combine(base.GetModulesFolderPath(), "ExportErpOrenMutagimAdapter.ini");            
        }
        ...
        protected override void RunExportInner(ExportErpCommandInfo info)
        {
			...
			string fileName = String.Format("SFIRADIMEX"); //result file only name without extension "LadyComfot_" & CodeERP & "_" (InventorDate)DDMMYYYY & ".CSV   
			...
			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductOrenMutagimERPFileProvider.ToString());
			...
			provider.Parms[ImportProviderParmEnum.InventorDate] = inventorDateFile1;
