...
namespace Count4U.ExportErpTextAdapter.RetalixNext
{
    public class ExportErpRetalixNextAdapterViewModel : ExportErpMakatViewModel
    {

		public ExportErpRetalixNextAdapterViewModel(IContextCBIRepository contextCbiRepository,
                                               ILog logImport,
        ...
        {
            return Path.Combine(base.GetModulesFolderPath(), "ExportErpRetalixNextAdapter.ini");
        }
        ...
        protected override void RunExportInner(ExportErpCommandInfo info)
        {
			...
			string fileName = String.Format("Sfira_{0}", branchCodeErp); //result file only name with extension            
			string fileNameWithExtension = String.Format("{0}.dta", fileName);
            string fullPath = Path.Combine(base.PathToExportErp, fileNameWithExtension);

			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductRetalixNextERPFileProvider.ToString());
			...
			IExportERPProvider provider2 = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductRetalixNextERPFileProvider1.ToString());
