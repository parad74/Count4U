...
{
    public class ExportErpMiniSoftAdapterViewModel : ExportErpMakatViewModel
    {

        public ExportErpMiniSoftAdapterViewModel(IContextCBIRepository contextCbiRepository,
                                               ILog logImport,
        ...
        {
            return Path.Combine(base.GetModulesFolderPath(), "ExportErpMiniSoftAdapter.ini");
        }
        ...
        protected override void RunExportInner(ExportErpCommandInfo info)
        {
            ...
            if (base.CurrentInventor != null)
            {
                ...
                inventorDate1 = dt.ToString("dd") + @"/"+ dt.ToString("MM") + @"/" + dt.ToString("yy") + "00:00";
            }

		
            //FILE1
			string fileName1 = String.Format("IMOVEIN_{0}_{1}", branchCodeErp, inventorDate); //result file only name without extension            
			...
			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductMiniSoftERPFileProvider.ToString());
			...
			IExportERPProvider provider1 = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductMiniSoftERPFileProvider1.ToString());
			...
			string fileName3 = String.Format("COUNT_{0}_{1}", branchCodeErp, inventorDate); //result file only name without extension            
			...
			//FILE3
			IExportERPProvider provider2 = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductMiniSoftERPFileProvider2.ToString());
