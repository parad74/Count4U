...
namespace Count4U.ExportErpTextAdapter.Nibit
{
    public class ExportErpNibitAdapterViewModel : ExportErpMakatViewModel
    {

		public ExportErpNibitAdapterViewModel(IContextCBIRepository contextCbiRepository,
                                               ILog logImport,
        ...
        {
			return Path.Combine(base.GetModulesFolderPath(), "ExportErpNibitAdapter.ini");
        }
        ...
        protected override void RunExportInner(ExportErpCommandInfo info)
        {
			...
			string fileName = String.Format("Sfira{0}", branchCodeErp); //result file only name with extension            
			string fileNameWithExtension = String.Format("{0}.msf", fileName);
            string fullPath = Path.Combine(base.PathToExportErp, fileNameWithExtension);

			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductNibitERPFileProvider.ToString());
			...
			IExportERPProvider provider2 = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductNibitERPFileProvider1.ToString());
