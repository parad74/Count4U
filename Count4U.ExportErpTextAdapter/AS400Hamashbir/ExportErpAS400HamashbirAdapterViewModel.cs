...
{
	public class ExportErpAS400HamashbirAdapterViewModel : ExportErpMakatViewModel
    {

		public ExportErpAS400HamashbirAdapterViewModel(IContextCBIRepository contextCbiRepository,
                                               ILog logImport,
        ...
        {
			return Path.Combine(base.GetModulesFolderPath(), "ExportErpAS400HamashbirAdapter.ini");
        }
        ...
        protected override void RunExportInner(ExportErpCommandInfo info)
        {
            ...
            if (base.CurrentBranch != null)
                branchCodeErp = base.CurrentBranch.BranchCodeERP.PadLeft(4,'0');
            ...
            if (base.CurrentInventor != null)
            {
				...
				inventorDate1 = dt.ToString("ddMMyyHH");// +dt.ToString("HHmm");
				//inventorDate2 = dt.ToString("ddMMyyHH");
            }

            //FILE1
			 //FromNextLine 00353.txt (without the date)
			//string fileName = String.Format("FromNextLine{0}", branchCodeErp); //result file only name with extension           
			string fileName = "MPRINSFIRA";
			...
			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductAS400HamashbirERPFileProvider.ToString());
			...
			IExportERPProvider provider2 = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductAS400HamashbirERPFileProvider1.ToString());
