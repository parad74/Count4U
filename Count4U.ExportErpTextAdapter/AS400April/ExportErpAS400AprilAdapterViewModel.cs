...
{
    public class ExportErpAS400AprilAdapterViewModel : ExportErpMakatViewModel
    {
		public ExportErpAS400AprilAdapterViewModel(IContextCBIRepository contextCbiRepository,
            ILog logImport,
        ...
        {
			return Path.Combine(base.GetModulesFolderPath(), "ExportErpAS400AprilAdapter.ini");            
        }
        ...
        protected override void RunExportInner(ExportErpCommandInfo info)
        {
            ...
            if (base.CurrentInventor != null)
            {
				...
				inventorDateFile = dt.ToString("ddMMyyyy");
				inventorDate = dt.ToString("yyyyMMdd");
            }

			string fileName = String.Format("SFIRA{0}", branchCodeErp); //result file only name without extension    
			string fileNameWithExtension = String.Format("SFIRA.{0}", branchCodeErp); //result file only name without extension    
			//string fileNameWithExtension = String.Format("{0}.FIN", fileName);
            string fullPath = Path.Combine(base.PathToExportErp, fileNameWithExtension);

			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductAS400AprilERPFileProvider.ToString());
			...
			//FILE2
			string fileName2 = String.Format("NF_INV_COUNT{0}_{1}", branchCodeErp, inventorDateFile); //result file only name with extension            
			...
			IExportERPProvider provider1 = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductAS400AprilERPFileProvider1.ToString());
