...
namespace Count4U.ExportErpTextAdapter.GazitLeeCooper
{
	public class ExportErpGazitLeeCooperAdapterViewModel : ExportErpMakatViewModel
    {

		public ExportErpGazitLeeCooperAdapterViewModel(IContextCBIRepository contextCbiRepository,
                                               ILog logImport,
        ...
        {
			return Path.Combine(base.GetModulesFolderPath(), "ExportErpGazitLeeCooperAdapter.ini");
        }
        ...
        protected override void RunExportInner(ExportErpCommandInfo info)
        {
            ...
            if (base.CurrentBranch != null)
                branchCodeErp = base.CurrentBranch.BranchCodeERP.PadLeft(3,'0');
            string inventorDate = string.Empty;
			string nowDate1 = string.Empty;
            if (base.CurrentInventor != null)
            {
                DateTime dt = base.CurrentInventor.InventorDate;
				inventorDate = dt.ToString("ddMMyyyy"); //DDMMYYYY
				DateTime dt1 = DateTime.Now;
				nowDate1 = dt1.ToString("ddMMyyyy"); //DDMMYYYY
				//inventorDate1 = dt.ToString("yyyyMMdd");// +dt.ToString("HHmm");
            }

            //FILE1
			 //FromNextLine 00353.txt (without the date)
			int len = branchCodeErp.Length;
			string branchCodeErp2 = "";
			if (len > 2) branchCodeErp2 = branchCodeErp.Substring(len - 2, 2);
			branchCodeErp2 = branchCodeErp2.PadLeft(2, '0');

			string fileName = String.Format("Sfira {0}", branchCodeErp2); //result file only name with extension            
			...
			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductGazitLeeCooperERPFileProvider.ToString());
			...
			//IExportERPProvider provider2 = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductAS400MegaERPFileProvider1.ToString());
