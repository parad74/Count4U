...
namespace Count4U.ExportErpTemplateAdapter.NativPlusYes
{
    public class ExportErpNativPlusYesAdapterViewModel : ExportErpMakatViewModel
    {
		...
		public ExportErpNativPlusYesAdapterViewModel(IContextCBIRepository contextCbiRepository,
                                               ILog logImport,
...
//2. Add current date to file name (_08-11-2017)
			string fileName1 = String.Format("NATIV_PLUS-INV_{0}_{1}_{2}", inventorDate2, branchCodeErp, inventorDate3); //result file only name without extension            
			...
			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductNativPlusYesProvider1.ToString());
			...
			string iturPath1_Q = Path.Combine(base.PathToExportErp, "EEEE" + fileNameWithExtension1_Q);


			IExportERPProvider provider_Q = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductNativPlusYesProvider1_Q.ToString());
			...
			{
				provider_Q.ToPathFile = iturPath1_Q;
			...
			{
				provider_Q.Export();
				//provider_Q.Export(info.IsFull, info.IsFilterByLocations, info.LocationCodeList, true, listIturCode);
			}
			...
			string iturPath1_SN = Path.Combine(base.PathToExportErp, "EEEE" + fileNameWithExtension1_SN);

			IExportERPProvider provider_SN = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductNativPlusYesProvider1_SN.ToString());
			...
			if (info.IsFilterByIturs == true && info.IsFull == false)
			{
				provider_SN.ToPathFile = iturPath1_SN;
				provider_SN.Export(info.IsFull, info.IsFilterByLocations, info.LocationCodeList, info.IsFilterByIturs, info.IturCodeList);
			}
			if (info.IsFull == true && info.IsFilterByIturs == false)
			{
				provider_SN.Export();
				//provider_SN.Export(info.IsFull, info.IsFilterByLocations, info.LocationCodeList, true, listIturCode);
			}
			...
			IExportERPProvider provider2 = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductNativPlusYesProvider2.ToString());
