...
namespace Count4U.ExportErpTextAdapter.GeneralXslx
{
	public class ExportErpGeneralXslxAdapterViewModel : ExportErpMakatViewModel
    {
		public ExportErpGeneralXslxAdapterViewModel(IContextCBIRepository contextCbiRepository,
            ILog logImport,
        ...
        {
			return Path.Combine(base.GetModulesFolderPath(), "ExportErpGeneralXslxAdapter.ini");            
        }
        ...
        protected override void RunExportInner(ExportErpCommandInfo info)
        {
			...
			//FILE1
			//KLITA_XXX.XLSX
			string fileName = String.Format("KLITA_{0}", branchCodeErp); //result file only name without extension    
			...
			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductGeneralXslxFileProvider.ToString());
			...
			provider.Parms[ImportProviderParmEnum.PriceCode] = priceCode;
			...
			////FILE2
			//string fileName1 = String.Format("KLITA_F_{0}", branchCodeErp); //result file only name without extension    
