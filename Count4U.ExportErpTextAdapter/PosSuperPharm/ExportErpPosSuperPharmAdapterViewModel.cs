...
namespace Count4U.ExportErpTextAdapter.PosSuperPharm
{
    public class ExportErpPosSuperPharmAdapterViewModel : ExportErpMakatViewModel
    {     
        public ExportErpPosSuperPharmAdapterViewModel(IContextCBIRepository contextCbiRepository,
                                               ILog logImport,
        ...
        {
            return Path.Combine(base.GetModulesFolderPath(), "ExportErpPosSuperPharmAdapter.ini");
        }
        ...
        protected override void RunExportInner(ExportErpCommandInfo info)
        {
            ...
            //FILE1
//			 Old format:
//FromNextLineXXX_DDMMYYYY.txt

//Need to be:
//FrominfoXXX.txt (XXX-ERP Branch code)

			string fileName1 = String.Format("Frominfo{0}", branchCodeErp); //result file only name without extension            
			...
			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductNetPOSSuperPharmERPFileProvider.ToString());
			...
			IExportERPProvider provider1 = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductNetPOSSuperPharmERPFileProvider1.ToString());
