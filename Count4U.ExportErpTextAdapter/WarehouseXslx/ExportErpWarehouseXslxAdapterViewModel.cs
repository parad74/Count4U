...
namespace Count4U.ExportErpTextAdapter.WarehouseXslx
{
    public class ExportErpWarehouseXslxAdapterViewModel : ExportErpMakatViewModel
    {
		public ExportErpWarehouseXslxAdapterViewModel(IContextCBIRepository contextCbiRepository,
            ILog logImport,
        ...
        {
			return Path.Combine(base.GetModulesFolderPath(), "ExportErpWarehouseXslxAdapter.ini");            
        }
        ...
        protected override void RunExportInner(ExportErpCommandInfo info)
        {
			...
			string fileName = String.Format("WarehouseINV_{0}", branchCodeErp); //result file only name without extension    
			...
			IExportERPProvider provider = base.ServiceLocator.GetInstance<IExportERPProvider>(ExportProviderEnum.ExportInventProductWarehouseXslxFileProvider.ToString());
